...
func (bms *deviceBuddyMemoryState) GetFreeMemorySize() uint64 {
	freeMemory := uint64(0)
	for level, fList := range bms.freeList {
		blockSize := bms.sizeOfLevel(level)
		freeMemory += uint64(fList.Len()) * blockSize
	}
	return freeMemory
}
